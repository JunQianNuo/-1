"""Problem 2 free-N search -- accelerated-copy entry point.

This script is copied from ``run_q2_free_search.py`` and edited only for
Problem 2 -> Problem 3 joint inverse expansion.  The original script and its
default result directory are intentionally left untouched.
...
import numpy as np
import q2_constellation_accel as q2
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=ROOT / "results" / "q2_free_search_accel")
    ap.add_argument(
        "--coverage-backend",
        choices=("numpy", "numba", "auto"),
        default="numpy",
        help="Backend for the copied evaluator. Default keeps safe NumPy semantics; numba is experimental.",
    )
    args = ap.parse_args()

    q2.set_coverage_backend(args.coverage_backend)
    ...
    print(f"[accelerated-copy] coverage_backend={q2.coverage_backend_name()} out={out}", flush=True)
