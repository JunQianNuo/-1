...
> [!info] 基准更正说明
> 本文写作时把 Walker 交付答案当作 $S{=}1512$（几何锁定）。后经 **$N$ 放开搜索**更正为 **第 2 问 $S_1{=}1320$（$55\times24$）、第 3 问 $S_2{=}1480$（$37\times40$）**（见 [[数学建模/第一次/问题分析/星链系统-文献驱动版/10-问题二数值实现方案.md|10]] §7）。下文出现的 $1512$ 为当时基准；**BILP 结论方向不变**（LP 下界 $\approx1010$、BILP 系列不实用），"Walker 参照数"应读作 $1320$（单重）。

> 目的：尝试 Jeon 2025 / Deng 2021 的 **0-1 整数规划（BILP）最少卫星**思路作为 Walker 枚举的替代，并借 **LP 松弛下界**回答"最小星数离下界多远"。
...
- **保留结论**：问题二交付答案取 **$N$ 放开的 $S_1{\approx}1320$（单）/ $S_2{\approx}1480$（双）**；BILP 的贡献是**下界 ≈1010** 与"非对称原则上可再省一点、但下界封顶"的判断。
...
**最终判断**：BILP 系列（2D 集合覆盖、CGT 1D）在本题都**不能替代 Walker**——2D 版精确不可解、CGT 版对宽区域无解。**唯一可迁移的价值是 LP 松弛下界 ≈1010**，它把 Walker 的 1512 定位在"下界 1.5 倍、且不可能低于 ~1010"。交付答案维持 **$N$ 放开的 1320/1480**。
